                ...
                {s.totaal != null && (
                  <div className="grid grid-cols-4 gap-2 mb-3">
                    <div onClick={e => { e.stopPropagation(); onSelectWithFilter(v.id, 'uitgeleend') }} className="cursor-pointer hover:opacity-70 transition-opacity">
                      <div className="text-xl font-bold text-sky-600">{s.uitgeleend || 0}</div>
                    ...
                    <div onClick={e => { e.stopPropagation(); onSelectWithFilter(v.id, 'vrij') }} className="cursor-pointer hover:opacity-70 transition-opacity">
                      <div className="text-xl font-bold text-emerald-500">{s.vrij || 0}</div>
                ...
                {s.totaal > 0 && (
                  <div className="flex items-center gap-2">
                    <div className="flex-1 bg-slate-100 dark:bg-slate-700 rounded-full h-2 overflow-hidden">
                      {bezetting > 0 && (
                        <div className="h-full bg-sky-500 transition-all" style={{ width: `${bezetting}%` }} />
                      )}
                    </div>
...
function VestigingTabs({ vestigingen, activeId, onChange, canGoBack }) {
  return (
    <div className="flex gap-1 px-4 pt-2 pb-0 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 overflow-x-auto">
      ...
      {/* Back to overview — only when multiple vestigingen accessible */}
      {canGoBack && (
...
export default function Uitleenoverzicht() {
  ...
  const isConcierge = !user?.is_beheerder
  ...
  function selectVestiging(id) {
    if (id === null && isConcierge && vestigingen.length === 1) {
      // Concierge with 1 vestiging cannot go back to picker
      return
    }
    if (id === null) {
      ...
      {hasVestiging && (
        <>
          <VestigingTabs vestigingen={vestigingen} activeId={filters.vestiging_id} onChange={selectVestiging}
            canGoBack={!(isConcierge && vestigingen.length === 1)} />
